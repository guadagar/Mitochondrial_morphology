...
'''
This script computes the average over 10 different initial conditions
4.02.20
GCG
...
npoints = int(1e5)#int(1e5) #time points available
...
for i in range(seed_number):
    ...
    if i>8:
        for s,j in enumerate(li):
            var = np.genfromtxt("./ra_cu/react_data/param1_000"+str(i+1)+j, dtype = float)
            arr[s,:,i]= var[:npoints,1]
            time[s,:] = var[:npoints,0]

    else:
        for s,j in enumerate(li):
            var = np.genfromtxt("./ra_cu/react_data/param1_0000"+str(i+1)+j, dtype = float)
